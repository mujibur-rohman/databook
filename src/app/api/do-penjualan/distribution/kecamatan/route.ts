...
// GET - Kecamatan distribution data with percentages
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedResults = results
      .filter((row) => row.kecamatan && Number(row.quantity) > 0)
      .map((row) => {
    ...
    console.error("Kecamatan distribution error:", error);
