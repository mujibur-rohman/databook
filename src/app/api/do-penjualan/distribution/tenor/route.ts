...
// GET - Tenor distribution data with percentages
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedResults = results
      .filter((row) => row.tenor !== null && Number(row.quantity) > 0)
      .map((row) => {
        const quantity = Number(row.quantity);
        const percentage =
          totalQuantity > 0 ? (quantity / totalQuantity) * 100 : 0;
        return {
          name: row.tenor?.toString() || "0",
          quantity: quantity,
    ...
    console.error("Tenor distribution error:", error);
    return NextResponse.json(
      { error: "Terjadi kesalahan saat mengambil data tenor" },
      { status: 500 }
    );
  }
}
