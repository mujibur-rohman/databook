...
// GET - Sales Force distribution data with percentages
export async function GET(request: NextRequest) {
  try {
    ...
    let query = db
      ...
      .groupBy(doPenjualan.salesForce)
    ...
    const formattedResults = results
      .filter((row) => row.salesForce && Number(row.quantity) > 0)
      .map((row) => {
    ...
    console.error("Sales Force distribution error:", error);
    return NextResponse.json(
      { error: "Terjadi kesalahan saat mengambil data sales force" },
      { status: 500 }
    );
  }
}
