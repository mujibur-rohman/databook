...
// GET - Kota distribution data with percentages
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedResults = results
      .filter((row) => row.kota && Number(row.quantity) > 0)
      .map((row) => {
    ...
    console.error("Kota distribution error:", error);
