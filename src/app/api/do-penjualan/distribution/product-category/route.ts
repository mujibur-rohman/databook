...
// GET - Product Category distribution data with percentages
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedResults = results
      .filter((row) => row.productCategory && Number(row.quantity) > 0)
      .map((row) => {
    ...
    console.error("Product Category distribution error:", error);
