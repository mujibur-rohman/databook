...
import { db } from "@/db";
import { doPenjualan, types, series as seriesTable } from "@/db/schema";
import { eq, and, gte, lte, sql, inArray } from "drizzle-orm";

// GET - Series distribution data with percentages
export async function GET(request: NextRequest) {
  try {
    ...
    let query = db
      ...
      .leftJoin(seriesTable, eq(types.seriesId, seriesTable.id))
    ...
    const formattedResults = results
      .filter((row) => row.seriesName && Number(row.quantity) > 0)
      .map((row) => {
    ...
    console.error("Series distribution error:", error);
    return NextResponse.json(
      { error: "Terjadi kesalahan saat mengambil data series" },
      { status: 500 }
    );
  }
}
