...
export const useSellInMonthly = (params: MonthlyAnalyticsParams) => {
  return useQuery({
    queryKey: ["sell-in-monthly", params],
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/sell-in/monthly-analytics?${searchParams}`,
        {
          credentials: "include",
        }
      );
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch Sell-In monthly analytics data");
      }
