...
export const useStuMonthly = (params: MonthlyAnalyticsParams) => {
  return useQuery({
    queryKey: ["stu-monthly", params],
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/stu/monthly-analytics?${searchParams}`,
        {
          credentials: "include",
        }
      );
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch STU monthly analytics data");
      }
