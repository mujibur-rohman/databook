...
export const useSpkMonthly = (params: MonthlyAnalyticsParams) => {
  return useQuery({
    queryKey: ["spk-monthly", params],
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/spk/monthly-analytics?${searchParams}`,
        {
          credentials: "include",
        }
      );
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch SPK monthly analytics data");
      }
