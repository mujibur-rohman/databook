...
interface UseDoPenjualanAnalyticsParams {
  startDate?: string;
  endDate?: string;
  branchIds?: number[];
  typeIds?: number[];
...
export const useDoPenjualanAnalytics = (
  params: UseDoPenjualanAnalyticsParams
) => {
  return useQuery({
    queryKey: ["do-penjualan-analytics", params],
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/do-penjualan/analytic?${searchParams}`,
        {
          credentials: "include",
        }
      );
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch DO Penjualan analytics");
      }
